...
We still have some import shortcuts for users, making everything in the `electrodes` subpackage (the contents of lfp, spiking, and probes modules) available under `cleo.electrodes`. We do this by importing the submodules' contents in `__init__.py` files. We can then test the shorcut imports by making sure to use them in the unit tests. However, we must use the full import path in the source code itself to avoid circular import errors. 