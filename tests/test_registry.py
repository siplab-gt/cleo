...
def test_multi_channel(sim_ng1_ng2, ops1):
    ...
    sim.reset()
    light.update([10, 10, 0, 0] * mwatt / mm2)
    sim.run(0.3 * ms)
    v_base_x = np.array(ng1.v)

    sim.reset()
    light.update([0, 0, 10, 10] * mwatt / mm2)
    sim.run(0.3 * ms)
    v_yz = np.array(ng1.v)
...
def test_multi_light_opsin(sim_ng1_ng2):
    ...
    blue.update(10 * mwatt / mm2)
    sim.run(0.3 * ms)
    # main effect on ng1
    assert chr2.epsilon(blue.wavelength) > vfchrimson.epsilon(blue.wavelength)
    assert np.all(ng1.Iopto > ng2.Iopto2)
    assert np.all(ng1.v > ng2.v)
    # some cross-talk on ng2
    assert np.all(ng2.v > -70 * mV)
    assert vfchrimson.epsilon(blue.wavelength) > 0
    assert np.all(ng2.Iopto2 > 0)
    v_crosstalk = ng2.v

    sim.reset()
    assert blue.value == 0
    amber.update(10 * mwatt / mm2)
    sim.run(0.3 * ms)
    # main effect on ng2
    assert chr2.epsilon(amber.wavelength) < vfchrimson.epsilon(amber.wavelength), (
        chr2.spectrum,
        vfchrimson.spectrum,
    )
    assert np.all(ng2.Iopto2 > ng1.Iopto)
    assert np.all(ng2.v > ng1.v)
    # very little cross-talk on ng1
    assert chr2.epsilon(amber.wavelength) < 0.01
    assert np.all(ng1.Iopto > 0)
    assert np.all((-70 * mV < ng1.v) & (ng1.v < v_crosstalk))

    sim.reset()
    uv.update(10 * mwatt / mm2)
    sim.run(0.3 * ms)
